...
SOURCE_CONTRACT_FIXTURE_DIR = TEST_DIR.join('SourceContractFixtures')
...
FileUtils.mkdir_p(SOURCE_CONTRACT_FIXTURE_DIR)
{
  'LabelTextCaptureSheet.swift.txt' => APP_DIR.join('Features/Scanner/LabelTextCaptureSheet.swift'),
  'ImageTextSelectionSheet.swift.txt' => APP_DIR.join('Features/Scanner/ImageTextSelectionSheet.swift'),
  'MenuViews.swift.txt' => APP_DIR.join('Features/Menu/MenuViews.swift'),
  'generate_project.rb.txt' => ROOT.join('scripts/generate_project.rb'),
  'FEATURES.md.txt' => ROOT.parent.join('docs/FEATURES.md'),
}.each do |fixture_name, source_path|
  File.write(SOURCE_CONTRACT_FIXTURE_DIR.join(fixture_name), File.read(source_path))
end
...
def add_tree(group, directory, target, resource_target: target)
  Dir.children(directory).sort.each do |entry|
    ...
    when '.xcassets', '.plist', '.strings', '.xcprivacy', '.txt'
