---
name: verify
description: Validate `app.js` syntax and confirm `APP_VERSION` in `core/domain/constants.js` matches the current `v0.8.x` branch before committing or releasing. Use when the user asks to verify, sanity-check, or run the old `/verify` workflow.
---
...
2. Read `APP_VERSION` from `core/domain/constants.js`.
