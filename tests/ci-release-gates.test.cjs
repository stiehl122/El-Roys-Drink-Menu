...
test('iOS launch gate only waits for Xcode Cloud when ios files change', () => {
  ...
  assert.match(workflow, /ios_sha=/);
  assert.match(workflow, /git log -n 1 --format=%H "\$base\.\.\$head" -- ios\//);
  assert.match(workflow, /SHA:\s*\$\{\{ steps\.changes\.outputs\.ios_sha \}\}/);
  ...
  assert.doesNotMatch(workflow, /--jq\s+--arg\b/);
});
